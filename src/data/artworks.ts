// Centralized artwork data — single source of truth for collections, artwork
// pages, sitemap generation and SEO metadata.
...
export const allArtworks: Artwork[] = raw.map((a) => ({ ...a, slug: slugify(a.title) }));
...
export function getArtworksByCollection(collection: string): Artwork[] {
  return allArtworks.filter((a) => a.collection === collection);
}

/**
 * Extract a short plain-text excerpt from the multiline description, suitable
 * for meta description (~155 chars).
 */
export function artworkExcerpt(a: Artwork, max = 155): string {
  const text = (a.description || "").replace(/\s+/g, " ").trim();
